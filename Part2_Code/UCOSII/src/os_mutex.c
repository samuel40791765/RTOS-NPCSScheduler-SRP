...
OS_EVENT  *OSMutexCreate (INT8U prio, INT8U *perr, INT8U highest)
{
    OS_EVENT  *pevent;
#if OS_CRITICAL_METHOD == 3                                /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr = 0;
...
/*$PAGE*/
    ...
    if ((INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) {
        ...
        //if this is not zero, this means there are other resoures not let go of yet, so we record the old ceiling
        if(SYSTEM_CEILING != 0){ 
